...
function randomWord() {
  ...
  const seed = parseInt(currentDate.replace(/-/g, ""), 10);
  const randomIndex = seed % options.length;
  return options[randomIndex];
}

// Le contenu du fichier README à générer
function generateReadmeContent() {
  const randomWordResult = randomWord();
  return `

[randomWord]: ${randomWordResult}

# ${randomWordResult} there and welcome to my GitHub 👋
...
Last update on ${formatDate(new Date())}
...
// Le nom du fichier README à créer
...
fs.writeFile(fileName, readmeContent, (err) => {
  if (err) {
    console.error(
      "Une erreur s'est produite lors de la création du fichier README :",
      err
    );
  } else {
    console.log("Le fichier README a été créé avec succès !");
  }
});
